...
Doing this also allowed me to hide the code (and output) so the student didn't have to see it at all. I realized this only changed the appearance on my end -- meaning a student would have to go in and manually hit "Hide code" for each cell -- but nonetheless, it's good to have the option, and perhaps this just acts as an incentive to expand this project further down the line, so a student can go through it without having to work inside a notebook. Overall, these adjustments largely fixed the previous frustrations I had with the flow and appearance of my tool, while keeping it all in the Google Colab notebook. While I'd like to build this tool out more -- and fully intend on doing so -- beyond this project, adding these lines of code and text chunks provided a quick(er) fix that improved the tool's presentation without increasing my workload too substantially (as building a website interface would have, for example). 
